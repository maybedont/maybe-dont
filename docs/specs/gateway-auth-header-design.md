...
export MAYBE_DONT_REQUIRED_HEADER_VALUE="*@example.com,service-account-1"
```
...
1. **Glob pattern** (contains `*`): Wildcard matching where `*` matches one or more characters
   - `*@example.com` matches `user@example.com`, `service@example.com`
   - `*@example.com` does NOT match `@maybedont.ai` (requires at least one character)
...
```
INFO  Required header authentication enabled  header=X-MaybeDont-Caller  allowed_values=[*@example.com, service-account-1]
```
...
```
INFO  Session initialized  session_id=abc123  caller=user@example.com
```
...
func TestAuthMiddleware_GlobPattern(t *testing.T) {
    ...
    // "*@example.com" does NOT match "@maybedont.ai" (requires 1+ char)
}

func TestAuthMiddleware_MultipleValues(t *testing.T) {
    // Comma-separated: "*@example.com,service-account-1"
...
func TestAllowedValue_Matches(t *testing.T) {
    ...
    // - "*@example.com" matches "user@example.com" ✓
    // - "*@example.com" matches "x@maybedont.ai" ✓
    // - "*@example.com" does NOT match "@maybedont.ai" ✗
    // - "*@example.com" does NOT match "dan@other.com" ✗
...
```json
{
  "mcpServers": {
    "maybe-dont": {
      ...
      "headers": {
        "X-MaybeDont-Caller": "user@example.com"
      }
    }
  }
}
