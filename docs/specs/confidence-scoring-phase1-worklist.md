...
- [ ] **1. AI request rules** (`internal/config/defaults/ai_request_rules.yaml`)
...
- [ ] **2. AI response rules** (`internal/config/defaults/ai_response_rules.yaml`)
...
- [ ] **3. AI policy authoring skill** (`internal/skills/ai-policy.md` and any variants in `.claude/skills/`)
...
- [ ] **4. Fix decision logic** (`internal/gateway/ai_response_engine.go` ~line 256-263)
  - Current (buggy):
    ```go
...
- [ ] **5. Add/update tests for redact logic** (`internal/gateway/ai_response_engine_test.go`)
...
- [ ] **6. `make test`** — all existing unit tests pass
...
- [ ] **10. Add empty response guard** (`internal/gateway/gateway.go` ~line 559)
  - Change:
    ```go
...
- [ ] **11. Add test for empty response skip** (`internal/gateway/gateway_test.go` or appropriate test file)
