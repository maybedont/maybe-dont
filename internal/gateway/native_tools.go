...
const (
	...
	// ToolDiscoverTools triggers lazy discovery for pass-through-auth downstream clients.
	ToolDiscoverTools = "maybedont__discover_tools"
	// ToolGenerateAuditReport runs an AI-powered security analysis over recent audit log entries.
	ToolGenerateAuditReport = "maybedont__generate_audit_report"
	// ToolGetAuditLog returns audit log entries with filtering and pagination.
	ToolGetAuditLog = "maybedont__get_audit_log"
	// ToolListDownstreamServers lists configured downstream MCP servers.
	ToolListDownstreamServers = "maybedont__list_downstream_servers"
	// ToolListSessions lists active client sessions.
