...
// Error category constants for consistent error classification across
// providers. These values appear in audit log entries as the error
// classification for a failed AI validation call.
const (
	// ErrCategoryAPIError is a provider API error not covered by a more specific category.
	ErrCategoryAPIError = "api_error"
	// ErrCategoryTimeout means the request exceeded its deadline.
	ErrCategoryTimeout = "timeout"
	// ErrCategoryCanceled means the request context was canceled (e.g. gateway shutdown).
	ErrCategoryCanceled = "canceled"
	// ErrCategoryParseError means the provider's response could not be parsed as the expected structured output.
	ErrCategoryParseError = "parse_error"
	// ErrCategoryNoResponse means the provider returned no usable response content.
	ErrCategoryNoResponse = "no_response"
	// ErrCategoryRateLimited means the provider rejected the request for exceeding a rate limit.
	ErrCategoryRateLimited = "rate_limited"
	// ErrCategoryAuthError means the provider rejected the configured API key/credentials.
	ErrCategoryAuthError = "auth_error"
	// ErrCategoryInvalidRequest means the request was malformed (e.g. bad model name, invalid parameters).
