...
const (
	ServerTypeSTDIO ServerType = "stdio" // Spawns a process and communicates via stdin/stdout
	ServerTypeHTTP  ServerType = "http"  // Standard HTTP requests/responses
	ServerTypeSSE   ServerType = "sse"   // Server-Sent Events for streaming connections
)
...
const (
	PolicyActionAllow  PolicyAction = "allow"  // Explicitly allow the request; other enabled rules in the phase still run
	PolicyActionDeny   PolicyAction = "deny"   // Block the request (when the rule's mode is "enforce"); first enabled deny ends the phase
	PolicyActionRedact PolicyAction = "redact" // Modify the response content rather than blocking it (response rules only)
)
...
type Policy struct {
	...
	// Expression is the CEL expression field predating the mcp_expression/cli_expression
	// split introduced in v1.1.0. New configs should use MCPExpression and/or CLIExpression
	// instead. At load time, if MCPExpression is empty, Expression is used as a fallback
	// for MCP validation.
