...
func (r *Runner) Run(ctx context.Context) (*RunResult, error) {
	...
	// Phase 3.1: Compute policy hashes for cache invalidation
	...
	if r.opts.StateFile != "" {
		...
		productVersion := r.opts.ProductVersion
		if productVersion == "" {
			productVersion = "dev"
		}
		r.stateManager, err = NewStateManager(r.opts.StateFile, r.suite.BundleID, productVersion, historyDepth)
		if err != nil {
...
func (r *Runner) executeAITests(ctx context.Context, cases []TestCase, models []ModelConfig, onProgress ProgressCallback) ([]TestResult, error) {
	...
	for _, model := range models {
		...
		runner, err := NewAITestRunner(model, r.suite, r.suiteDir, r.logger, r.rateLimiter, r.opts.ProviderClientFactory, r.opts.IncludeDisabled)
		if err != nil {
...
func (r *Runner) outputResults(results []TestResult, summary *RunResult, alreadyStreamed bool) error {
	...
	if r.opts.OutputFile != "" {
		...
		default: // json is default for file output
			loadedPolicies, err := r.loadAllPoliciesWithStatus()
			if err != nil {
				return fmt.Errorf("failed to load policies for JSON output: %w", err)
			}
			fileOutput, err = formatJSONOutput(r.suite, results, summary, coverage, comparison, loadedPolicies)
			if err != nil {
