...
func formatJSONOutput(suite *Suite, results []TestResult, summary *RunResult, coverage *CoverageReport, comparison []ModelComparisonEntry, loadedPolicies []policyInfo) (string, error) {
	...
	for _, p := range loadedPolicies {
		...
		switch p.engine {
		case "cel_request":
			policiesLoaded.CELRequest = append(policiesLoaded.CELRequest, p.name)
		case "ai_request":
			policiesLoaded.AIRequest = append(policiesLoaded.AIRequest, p.name)
		case "cel_response":
			policiesLoaded.CELResponse = append(policiesLoaded.CELResponse, p.name)
		case "ai_response":
			policiesLoaded.AIResponse = append(policiesLoaded.AIResponse, p.name)
		}
	}
