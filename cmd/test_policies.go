...
// Exit codes for `maybe-dont test policies`, stable for CI integration.
const (
	// ExitSuccess means all tests ran and passed thresholds.
	ExitSuccess = 0
	// ExitTestsFailed means tests ran but did not meet the configured pass-rate thresholds.
	ExitTestsFailed = 1
	// ExitSchemaValidation means a test case or suite file failed schema validation.
	ExitSchemaValidation = 2
	// ExitPolicyIntegrity means the loaded CEL/AI policy files don't match the suite's expected hash.
	ExitPolicyIntegrity = 3
	// ExitPathResolution means a configured path (suite dir, state file, policy file) could not be resolved.
	ExitPathResolution = 4
	// ExitMoreTestsRemain means incremental mode completed a batch but unrun test cases remain.
